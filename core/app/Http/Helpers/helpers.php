...
{
    $name = $type ? "/logo_$type.png" : '/logo_2.png';
    return getImage(getFilePath('logoIcon') . $name);
}
...
function durationTimeFormat($seconds)
{
    $hours   = floor($seconds / 3600);
    $minutes = floor(($seconds % 3600) / 60);
...
function ratingStar($rating = 0)
{
    ...
    for ($i = 0; $i < floor($rating); $i++) {
        $ratingStar .= '<span class="review-card-star review-card-star-full"></span>';
    }
    if (0 < $rating - floor($rating) && $rating - floor($rating) <= 0.25) {
        $ratingStar .= '<span class="review-card-star"></span>';
    } elseif (0.25 < $rating - floor($rating) && $rating - floor($rating) <= 0.75) {
        $ratingStar .= '<span class="review-card-star review-card-star-half"></span>';
...
function getVideoPath($lesson)
{
    if (!$lesson->path) {
        return false;
    }
    if ($lesson->server) {
        $path = @gs('ftp')->domain . $lesson->path;
    } else {
        $path = asset(getFilePath('video') . $lesson->path);
    }
    return $path;
}

function getAssetPath($lesson)
{
    if (!$lesson->asset_path) {
        return false;
    }
    if ($lesson->asset_server) {
        $path = @gs('ftp')->domain . $lesson->asset_path;
    } else {
        $path = asset(getFilePath('lesson_asset') . $lesson->asset_path);
    }
    return $path;
}
