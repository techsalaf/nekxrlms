...
class AdminController extends Controller
{
    ...
    public function dashboard()
    {
        ...
        $sold['today']            = CoursePurchase::whereDate('created_at', today())->sum('purchased_amount');
        $sold['last_seven_days']  = CoursePurchase::whereDate('created_at', '>', today()->subDays(7))->sum('purchased_amount');
        $sold['last_thirty_days'] = CoursePurchase::whereDate('created_at', '>', today()->subDays(30))->sum('purchased_amount');
        $sold['all_time']         = CoursePurchase::sum('purchased_amount');

        return view('admin.dashboard', compact('pageTitle', 'widget', 'chart','deposit','sold'));
    }
    ...
    public function transactionReport(Request $request) {
        ...
        $plusTransactions   = CoursePurchase::whereDate('created_at', '>=', $request->start_date)
            ->whereDate('created_at', '<=', $request->end_date)->with('course')
            ->selectRaw('SUM(purchased_amount) AS amount')
            ->selectRaw("DATE_FORMAT(created_at, '{$format}') as created_on")
            ->latest()
            ->groupBy('created_on')
            ->get();
    ...
    public function courseReviews()
    {
        ...
        $reviews   = Review::searchable(['course:title', 'user:username'])->with('course', 'user')->orderBy('id', 'desc')->paginate(getPaginate());

        return view('admin.course.reviews', compact('pageTitle', 'reviews'));
    }
    ...
    public function deleteCourseReview($id)
    {
        ...
        $course->total_rating -= $review->rating;
        $course->avg_rating = $course->total_review ? $course->total_rating / $course->total_review : 0;
        $course->save();
        $review->delete();

        $notify[]=['success','Course review deleted successfully'];
        return back()->withNotify($notify);
    }
